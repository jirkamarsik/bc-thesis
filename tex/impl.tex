...
\subsection{\class{RoughTokenizer}}
...
\subsection{\class{FeatureExtractor}}
...
\subsection{\class{OutputFormatter}}
...
\class{Classifier}. The \class{Classifier} is aligns this stream of rough
...
string in question be \example{abab} and the \maysplit{} regular expression
pair \example{a} - \example{b(ab)*}. Any attempt to search for the suffix
\example{b(ab)*} would yield the \example{bab} substring due to the leftmost
...
they made writing the transfer of text between the \class{TextCleaner}, the
...
reports them to the \class{OutputFormatter} and writes the transformed input to
an opipestream (an output pipestream). The \class{Encoder} on the other hand
